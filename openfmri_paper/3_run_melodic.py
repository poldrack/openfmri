...
outdir='/corral-repl/utexas/poldracklab/openfmri/analyses/paper_analysis_Dec2012/ICA'
...
for run in [1,2]:
  for c in ncomps:
    cmd='melodic -i %s/zstat_run%d_add10000.nii.gz -o %s/ica_run%d_%dcomp -v --report --Oall -d %d --nobet -m /corral-repl/utexas/poldracklab/software_lonestar/fsl-5.0.1/data/standard/MNI152_T1_2mm_brain_mask.nii.gz --bgimage=/corral-repl/utexas/poldracklab/software_lonestar/fsl-5.0.1/data/standard/MNI152_T1_2mm_brain.nii.gz'%(datadir,run,outdir,run,c,c)
    f.write(cmd+'\n')
f.close()


print "now run: launch -s run_melodic.sh -r 08:00:00 -n melodic -e 1way -p 72"
